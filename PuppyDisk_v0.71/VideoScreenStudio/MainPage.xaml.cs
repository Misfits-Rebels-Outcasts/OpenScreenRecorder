...
namespace VideoScreenStudio
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        private async void UnpackFiles_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                StartWritingOutput("Need to record a stream first");
                return;
            }
            else if (simpleRecorder.streamFile.IsAvailable ==false)
            {
                StartWritingOutput("Need to record a stream first");
                return;
            }
        ...
        public async Task UnpackToMp4()
        {
            ...
            if (simpleRecorder.videostream != null)
            {
                ...
                for (int i = 0; i < len; i++)
                {
                    ...
                    simpleRecorder.videostream.Seek(unpackItem.pos);
                    Windows.Storage.Streams.Buffer buffer = new Windows.Storage.Streams.Buffer((uint)unpackItem.length);
                    await simpleRecorder.videostream.ReadAsync(buffer, (uint)unpackItem.length, InputStreamOptions.None);
        ...
        private void CompletedReport(IAsyncOperationWithProgress<TranscodeFailureReason, double> asyncOp, AsyncStatus status)
        {
            ...
            Dispatcher.RunAsync(CoreDispatcherPriority.Normal, () =>
            {
                ...
                if (simpleRecorder!=null)
                {
                    ...
                    //Delete the intermediate stream file after it is unpacked
                    if (simpleRecorder.streamFile != null)
